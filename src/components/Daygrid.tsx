    ...
    <div className="flex flex-col px-3">
              ...
              <li className="list_disc_style">
                ...
                <ul className="flex flex-col gap-2 mt-2">
                  <li className="list_circle_style pl-7">
                    보다 적은 코드량으로 전역 상태 관리를 하기 위해 사용
                  </li>
                </ul>
              </li>
              <li className="list_disc_style">
                ...
                <ul className="flex flex-col gap-2 mt-2">
                  <li className="list_circle_style pl-7">
                    캐싱 기능을 통해 비동기 데이터의 중복 호출을 방지하고, 데이터를 지속적으로
                    동기화 하는 등 서버 데이터를 손쉽게 관리하기 위해 사용
                  </li>
                  <li className="list_circle_style pl-7">
      ...
      <ul className="flex flex-col ml-3 mb-7 text-lg gap-3">
        <li className="list_disc_style">
          ...
          <ul className="flex flex-col gap-2 mt-2">
            <li className="list_circle_style pl-7">
              Axios 인터셉터를 이용하여 각 요청마다 Access token을 자동 삽입하며, 토큰 만료 시
              Refresh token으로 토큰 재발급을 받을 수 있도록 설정
            </li>
            <li className="list_circle_style pl-7">
              보안 강화를 위한 토큰 설정
              <ul className="flex flex-col gap-2 mt-2">
                <li className="list_circle_style pl-7">
                  Access token은 외부 접근이 어려운 로컬 변수에, Refresh token은 브라우저에서 접근이
                  불가능한 HTTP Only Cookies에 저장
                </li>
                <li className="list_circle_style pl-7">
                  Access token의 유효기간을 1시간으로 짧게 설정하여, 토큰이 악용될 가능성을 최소화
                </li>
                <li className="list_circle_style pl-7">
                  Refresh token은 Access token 만료 시에만 서버로 전송하여 토큰 탈취 위험을 줄임
                </li>
              </ul>
            </li>
          </ul>
        ...
        <li className="list_disc_style">
          React-Query를 사용하여 마이페이지의 서버 데이터를 효율적으로 관리
          <ul className="flex flex-col gap-2 mt-2">
            <li className="list_circle_style pl-7">
              쉽게 변하지 않는 유저 정보는 시간 제한 없이 캐싱하여 불필요한 fetching을 방지
            </li>
            <li className="list_circle_style pl-7">
              사용자 정보가 수정되었을 경우, useMutation을 통해 서버 데이터를 업데이트하고, 캐시
              데이터를 무효화함. 이후 서버로부터 최신 데이터를 새로 가져오도록 설정하여 유저 정보를
              최신 상태로 유지함.
            </li>
          </ul>
        </li>
        <li className="list_disc_style">
          ...
          <ul className="flex flex-col gap-2 mt-2">
            <li className="list_circle_style pl-7">
              일정을 생성할 경우, 각 일정의 날짜와 시간에 맞추어 달력에 일정 표시가 생성될 수 있도록
              설계
            </li>
            <li className="list_circle_style pl-7">
              일정 표시의 position을 absolute로 설정하고, 날짜와 시간에 따라 left와 top 속성을
              조정하여 표시
            </li>
          </ul>
        </li>
        <li className="list_disc_style">
          ...
          <ul className="flex flex-col gap-2 my-2">
            <li className="list_circle_style pl-7">
              API 작성 시에는 자원과 행위를 명확하게 표현한 RESTful API로 작성
            </li>
          </ul>
        </li>
        <li className="list_disc_style">
          CRA를 사용하지 않고, Webpack과 Babel을 설정하여 프론트엔드 개발 환경 세팅
        </li>
      </ul>
      ...
      <div className="flex flex-col pb-7 mb-7 text-lg gap-3 border-b-one border-secondary-lightgray">
        ...
        <div className="mb-2">
          프로젝트 빌드 시간이 오래 걸려 배포를 하거나 개발 환경을 시작할 때 불편함을 겪었음
        </div>
        ...
        <ul className="flex flex-col gap-2">
          ...
          <li className="list-disc ml-5 pl-7">
            esbuild-loader가 타입 체킹을 진행하지 않아, fork-ts-checker-webpack-plugin을 사용하여
            타입 체킹을 별도의 프로세스로 분리함
          </li>
          <li className="list-disc ml-5 pl-7">
      ...
      <div className="flex flex-col pb-7 mb-7 text-lg gap-3 border-b-one border-secondary-lightgray">
        ...
        <div className="mb-2">
          같은 날짜에 일정을 여러개 등록할 경우, 달력에 생성된 일정 표시가 다른 일정 표시를 가리는
          현상 발생
        </div>
        <div className="font-medium">💡 해결 방법</div>
        <ul className="flex flex-col gap-2">
      ...
      <div className="flex flex-col mb-7 text-lg gap-3">
        ...
        <ul className="flex flex-col gap-2">
          <li className="list-disc ml-5 pl-7">
            인터넷 검색을 통해 CORS 에러가 브라우저의 동일 출처 정책 때문에 발생함을 알게 되었음.
            서버와 브라우저의 출처가 다르므로 보안상의 이유로 다른 출처에 접근이 제한되었음.
          </li>
          <li className="list-disc ml-5 pl-7">
            Access-Control-Allow-Origin에 브라우저의 출처를 명시하여 브라우저의 접근을 허용하도록
            설정
          </li>
          <li className="list-disc ml-5 pl-7">
            응답 헤더의 Access-Control-Allow-Credentials 옵션을 true로 설정하여, 브라우저가 응답에
            접근 가능하도록 하였음
          </li>
          <li className="list-disc ml-5 pl-7">
            Access-Controls-Allow-Methods에 브라우저에서 자원을 접근할 때 사용하는 모든 메서드를
            명시하여, HTTP 메서드에 상관 없이 접근을 허용
          </li>
        </ul>
      </div>
      ...
      <ol className="flex flex-col ml-3 mb-7 text-lg gap-3">
        <li className="list_decimal_style">
          ...
          <ul className="flex flex-col ml-3 mt-2 text-lg gap-3">
            <li className="list_disc_style">
              기획부터 디자인, 개발, 배포까지의 전 과정을 직접 진행하며, 프로덕트가 만들어지는
              과정을 경험하였습니다.
            </li>
          </ul>
        </li>
        <li className="list_decimal_style">
          ...
          <ul className="flex flex-col ml-3 mt-2 text-lg gap-3">
            <li className="list_disc_style">
              서버와 데이터베이스 구축, API 작성 등을 통해 백엔드의 작동 원리와 CORS 에러의 발생
              원인에 대해 더욱 깊이 이해할 수 있었습니다.
            </li>
          </ul>
        </li>
        <li className="list_decimal_style">
          ...
          <ul className="flex flex-col ml-3 mt-2 text-lg gap-3">
            <li className="list_disc_style">
              달력 구현 과정에서 라이브러리를 사용하지 않고 직접 구현해보면서, CSS의 flex와 grid
              개념 및 사용법을 확실히 익힐 수 있었습니다.
            </li>
          </ul>
        </li>
        <li className="list_decimal_style">
          ...
          <ul className="flex flex-col ml-3 mt-2 text-lg gap-3">
            <li className="list_disc_style">
              코드 컨벤션, 커밋 컨벤션 등의 팀 규칙과 프로젝트 진행상황을 문서화하는 과정을 통해
              기록의 중요성을 깨달았습니다. 이를 통해 프로젝트의 방향성을 파악하고, 팀원 간의
              의사소통이나 코드 리뷰에 드는 시간을 줄일 수 있었습니다.
            </li>
          </ul>
        </li>
      </ol>
